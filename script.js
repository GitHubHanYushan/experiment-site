...
function updateMemberList(containerId, members, groupId) {
    ...
    members.forEach((member, index) => {
        const progressText = member.progress[currentWeek] || '';
        const hasProgress = progressText.trim() !== '';
        
        // 添加项目主题显示（如果是科研或硬件课题组）
        let projectTopicHtml = '';
        if ((groupId === 'group-1' || groupId === 'group-2') && member.projectTopic) {
            projectTopicHtml = `<div class="member-project-topic">项目: ${member.projectTopic}</div>`;
        }
        ...
        memberElement.innerHTML = `
            ...
            <div class="member-progress">
                <div class="member-progress-bar">
                    <div class="member-progress-value" style="width: ${hasProgress ? '100%' : '0%'}"></div>
                </div>
                <div class="member-status">第${currentWeek}周进度: ${hasProgress ? '已记录' : '未记录'}</div>
            </div>
            ${hasProgress ? `<div class="member-content">${progressText}</div>` : ''}
        `;
        ...
        // 如果是微信小程序开发组，显示小程序信息
        if (groupId === 'group-4' && (member.miniprogramLink || member.qrcodeImage)) {
            const miniprogramInfo = document.createElement('div');
            miniprogramInfo.className = 'miniprogram-info';
            
            // 添加小标题
            const infoTitle = document.createElement('h4');
            infoTitle.textContent = '小程序信息';
            miniprogramInfo.appendChild(infoTitle);
            
            // 添加链接（如果有）
            if (member.miniprogramLink) {
                const link = document.createElement('a');
                link.href = member.miniprogramLink;
                link.target = '_blank';
                link.textContent = member.miniprogramLink;
                miniprogramInfo.appendChild(link);
            }
            
            // 添加二维码（如果有）
            if (member.qrcodeImage) {
                const qrcodeContainer = document.createElement('div');
                qrcodeContainer.className = 'miniprogram-qrcode';
                
                const qrcodeImg = document.createElement('img');
                qrcodeImg.src = member.qrcodeImage;
                qrcodeImg.alt = `${member.name}的小程序二维码`;
                
                qrcodeContainer.appendChild(qrcodeImg);
                miniprogramInfo.appendChild(qrcodeContainer);
            }
            
            memberElement.appendChild(miniprogramInfo);
        }
    });
}
